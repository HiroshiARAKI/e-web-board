# Keinage Architecture

## 概要

Keinage は**表示画面（ボード）**と**管理画面（ダッシュボード）**の 2 つのビューで構成されるフルスタック Web アプリケーションです。  
...
### スライドショー画像プリロード (v1.3.0)

低スペック端末 (Raspberry Pi 4 等) では、次画像のデコードとスライド遷移アニメーションが競合し、部分表示やちらつきが発生していた。  
以下の最適化を `MediaSlider` コンポーネントに適用した。

| 手法 | 詳細 |
|------|------|
| **先読み (Prefetch)** | 現在のスライドが表示された時点で、次の最大 2 枚の画像を `new Image()` でバックグラウンドフェッチし、ブラウザキャッシュに載せる |
| **onLoad ゲート** | 画像の `onLoad` イベントが発火するまでスライド切替タイマーを開始しない。これによりデコード完了前の遷移を防止 |
| **動画との分離** | 動画は `onEnded` イベントによる遷移を維持しつつ、フォールバック用タイマーを別 effect で管理 |

プリロード数 (`PRELOAD_AHEAD`) は定数化しており、将来的にネットワーク品質に応じた動的調整にも拡張可能。  
サーバー側のアップロードファイル配信ルート (`/uploads/[...path]`) は `Cache-Control: public, max-age=31536000, immutable` を返しているため、一度プリロードされた画像は再フェッチされない。
