...
Keinage は予約を検知した時点で、移行先プランの上限に収まる有効ボード候補を自動選択します。優先順位は「直近で表示されたボード」「最近更新されたボード」「最近作成されたボード」です。ユーザーは実際の切替日までは Billing 画面から候補を変更できます。

実際に下位プランへ切り替わると、候補に含まれるボードだけが有効状態で残り、それ以外はプラン都合の無効状態になります。候補が未設定または不正な場合でも、同じ優先順位で再選択してから適用します。

Billing 画面では、予約中の移行先プランに対して現在の使用量が超過する項目を表示します。対象はボード数、画像数、ストレージ、動画利用可否、動画解像度、1ファイル上限です。ダウングレード後に現在プランの上限を超えた状態になった場合も、管理画面で整理方法と上位プランへ戻す導線を表示します。
