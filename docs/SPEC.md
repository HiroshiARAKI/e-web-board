...
上位プランから下位プランへ変更予約し、作成済みボード数が新しいプランの上限を超える場合、管理者は「プランとお支払い」画面で移行後も有効にするボードを事前選択します。現在の請求期間中は現在のプラン制限を維持するため、全ボードを引き続き利用できます。

次回更新日に下位プランへ切り替わると、事前選択されなかったボードは削除されず、`inactive_due_to_plan` として保持されます。`inactive_due_to_plan` のボードは表示・編集できませんが、ボード一覧から削除できます。再アップグレードや削除で上限に空きができた場合は、同じ画面から再度有効化できます。Self-hosted / Unlimited mode ではこの制限は適用されません。
