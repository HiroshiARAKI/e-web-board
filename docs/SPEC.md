...
上位プランから下位プランへ変更し、作成済みボード数が新しいプランの上限を超える場合、管理者は「プランとお支払い」画面で有効にするボードを選択します。選択されなかったボードは削除されず、`inactive_due_to_plan` として保持されます。

`inactive_due_to_plan` のボードは表示・編集できませんが、ボード一覧から削除できます。再アップグレードや削除で上限に空きができた場合は、同じ画面から再度有効化できます。Self-hosted / Unlimited mode ではこの制限は適用されません。
