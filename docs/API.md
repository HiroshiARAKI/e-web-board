...
新規アップロードの storage key は Owner / board scope を含みます。public board のメディアは `CLOUDFRONT_BASE_URL` または `S3_PUBLIC_BASE_URL` が設定されている場合に CDN URL として返されます。private board のメディアは `/uploads/<path>` route 経由の認可配信を維持します。
