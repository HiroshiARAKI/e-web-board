...
- Google 認可 URL には `code_challenge` / `code_challenge_method=S256` / `nonce` を含めます。
...
- token exchange では保存済み PKCE `code_verifier` を送信します。
- token response の ID token payload で issuer、audience、有効期限、nonce を検証します。
