...
└──────────────────────┬───────────────────────┘
      ...
      ┌──────────────┼──────────────┐
      ▼              ▼              ▼
      PostgreSQL     Garage / uploads/   External APIs
      Drizzle ORM    thumbs/ fallback    weather.tsukumijima.net
                     GitHub Releases API
                     SMTP
```
...
- 実体の保存先は S3 互換ストレージを優先し、Garage を想定して `S3_*` 環境変数で接続します。
- `S3_*` 未設定時は `uploads/` と `uploads/thumbs/` を使うローカル保存にフォールバックします。
...
Next.js standalone 出力では、ビルド後に追加された `public/` 配下ファイルをそのまま配信できません。そのため Keinage では `src/app/uploads/[...path]/route.ts` を経由し、ローカルディスクまたは Garage 上のオブジェクトを同じ `/uploads/...` URL で返却します。
...
- `uploads/` : S3 未設定時のアップロードファイルとサムネイル
- Garage bucket : S3 設定時のアップロードファイルとサムネイル
