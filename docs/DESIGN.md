...
└──────────────────────┬───────────────────────┘
        ...
        ┌──────────────┼──────────────┐
        ▼              ▼              ▼
   SQLite (WAL)    uploads/      External APIs
   better-sqlite3  thumbs/       weather.tsukumijima.net
                                 GitHub Releases API
                                 SMTP
```
...
| `src/db` | DB スキーマと接続 |
| `src/lib` | 認証、SSE、画像処理などの共通ロジック |
| `drizzle/` | SQL マイグレーション |
| `docker/` | Dockerfile、migration runner、entrypoint |
| `data/` | SQLite DB ファイル置き場 |
| `uploads/` | 実アップロードファイル置き場 |
...
- 単純なインメモリ制御ではなく DB 記録にしているため、同一 DB を使う単一インスタンス運用では再起動後も履歴を参照できます。
...
- 実ファイルは `uploads/` に保存します。
- サムネイルは `uploads/thumbs/` に保存します。
...
Next.js standalone 出力では、ビルド後に追加された `public/` 配下ファイルをそのまま配信できません。そのため Keinage では `src/app/uploads/[...path]/route.ts` からローカルディスクの `uploads/` を直接読み出して返却します。
...
- Next.js build 時の並列モジュール評価で `better-sqlite3` への同時アクセスが起きうる
- import 時点で DB を開かず、最初の利用時に接続することで `SQLITE_BUSY` を避ける
