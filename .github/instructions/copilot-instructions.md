...
| DB | PostgreSQL (ex: Drizzle ORM + SQLite (better-sqlite3, WAL mode)) |
...
## 開発指針
- `main`ブランチは最新りリース版と一致させる。
- 新機能やバグ修正は `dev` ブランチで行い、安定したら `main` にマージしてリリースする。
- PRは必ず日本語で説明を書くこと。コードの変更点と理由を明確にする。
- Issue単位での対応では、必ず`dev`から新規にブランチを切って対応し、PR作成まで行うこと。直接`dev`にコミットしないこと。
